...
export default function UserEditModal({ isOpen, onClose, onSubmit, user, setUser, departments }: Props) {
        ...
        <div className="fixed inset-0 z-70 flex items-center justify-center">
            ...
            <form onSubmit={onSubmit} className="relative z-10 w-full max-w-2xl bg-white rounded-xl p-6 shadow-xl">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="font-headline text-xl text-[#00113a]">Edit User</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <input value={user.id} onChange={(event) => setUser((state) => ({ ...state, id: event.target.value }))} placeholder="ID" className="w-full bg-white border border-[#c5c6d2]/40 py-3 px-4 rounded-md font-label text-xs uppercase tracking-widest text-[#444650] placeholder:text-[#444650]/60 focus:ring-0 focus:border-[#775a19]" />
                ...
                <div className="mt-4 flex justify-end gap-2">
                    ...
                    <button type="submit" className="px-4 py-2 rounded-md bg-[#00113a] text-white">Save Changes</button>
                </div>
            </form>
        </div>
    );
